...
function Rules({ p, approximations }: { p: Report["params"]; approximations: string[] }) {
  ...
  return (
    <div className="grid gap-3 lg:grid-cols-2">
      <section className="rounded-2xl border border-line bg-[#0e1217]/[0.97] p-4">
...
function RunTable({ runs, empty }: { runs: Run[]; empty: string }) {
  if (runs.length === 0) return <p className="rounded-2xl border border-line bg-[#0e1217]/[0.97] px-4 py-10 text-center text-sm text-ink-3">{empty}</p>;
  return (
