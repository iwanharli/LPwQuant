...
async def get_brontosaurus() -> dict:
    """Paper test of the fee-farming style: wide spot ranges on high-fee memecoin pools, held ~12h."""
    if engine.db is None:
        raise HTTPException(status_code=503, detail="engine not ready")
    return await brontosaurus.report(engine.db, engine.rows)


@app.get("/api/paper/copy")
async def get_copy_paper() -> dict:
...
async def get_lp_leaders() -> dict:
    """Meteora LP wallets ranked on the same yardstick as the paper tests."""
    if engine.db is None:
        raise HTTPException(status_code=503, detail="engine not ready")
    return await lp_leaders.report(engine.db)
...
@app.get("/api/paper/screen")
async def paper_screen(profile: str = Query("satu_sisi")) -> dict:
    """The profile's view of the live pools: why pools fail it, and the entry checklist of those that pass."""
    return {"updated_at": engine.updated_at, **profile_screen.screen(engine.rows, _paper(profile))}
