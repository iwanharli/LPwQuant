...
def fee_for_position_pct_day(
    fee_pct_day_of_tvl: float, tvl_usd: float, position_usd: float, n_bins: int, pool_per_bin_usd: float
) -> float:
    """Daily fees for a position spread uniformly over `n_bins`, as % of the position.

    Pool fees per day in USD are fee/TVL x TVL. Only the bins within the trading window earn, so the position
    earns (per-bin size / (pool per-bin depth + per-bin size)) of them. With n_bins larger than the window
    the per-bin size, and so the share, shrinks: that is the real cost of a wide range.
    """
    if position_usd <= 0 or tvl_usd <= 0 or n_bins <= 0:
        return 0.0
    fees_usd_day = fee_pct_day_of_tvl / 100 * tvl_usd
    share = fee_share(position_usd / n_bins, pool_per_bin_usd)
